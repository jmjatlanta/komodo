...
# This builds an OSX executable for the M1 Mac architecture
# using the gnu cross-compiler on the linux architecture
# the compilers should already be in the $PATH (i.e. aarch64-linux-gnu-g++-9)
...
make HOST="${HOST}" "$@" -C ./depends/ V=1 NO_QT=1 NO_PROTON=1
...
./configure --prefix="${PREFIX}" --target="${TARGET}" --with-gui=no "$HARDENING_ARG" "$LCOV_ARG" "$DEBUGGING_ARG"
