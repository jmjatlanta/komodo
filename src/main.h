...
bool ContextualCheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &view, bool fScriptChecks,
        ...
        CTxMemPool& pool, std::vector<CScriptCheck> *pvChecks = NULL);
...
public:
    ...
    CScriptCheck(const CCoins& txFromIn, const CTransaction& txToIn, unsigned int nInIn, 
            ...
            PrecomputedTransactionData* txdataIn, CTxMemPool* pool) :
        ...
        consensusBranchId(consensusBranchIdIn), error(SCRIPT_ERR_UNKNOWN_ERROR), txdata(txdataIn),
        pool(pool) { }
