        ...
        bool (*isInitBlockDownload)(),int32_t validateprices)
{
    ...
    if (!saplingActive) {
        // Size limits
        //BOOST_STATIC_ASSERT(MAX_BLOCK_SIZE(chainActive.LastTip()->nHeight+1) > MAX_TX_SIZE_BEFORE_SAPLING); // sanity
...
bool CheckTransactionWithoutProofVerification(uint32_t tiptime,const CTransaction& tx, CValidationState &state)
{
    ...
    // Size limits
    //BOOST_STATIC_ASSERT(MAX_BLOCK_SIZE(chainActive.LastTip()->nHeight+1) >= MAX_TX_SIZE_AFTER_SAPLING); // sanity
...
namespace Consensus {
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if ( ASSETCHAINS_SYMBOL[0] == 0 && nSpendHeight > ::Params().KomodoInterestMinSpendHeight() )//chainActive.LastTip() != 0 && chainActive.LastTip()->nHeight >= 60000 )
            {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    */
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            nFees += (stakeTxValue= view.GetValueIn(chainActive.LastTip()->nHeight,&interest,tx,chainActive.LastTip()->nTime) - valueout);
            sum += interest;
    ...
    {
        if ( ASSETCHAINS_SYMBOL[0] != 0 || pindex->nHeight >= Params().Season1StartHeight() 
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( height > Params().Season3StartHeight() && ASSETCHAINS_SYMBOL[0] == 0 )
    {
        int32_t notaryid;
        int32_t special = komodo_chosennotary(&notaryid,height,pubkey33,tiptime);
        if (notaryid > 0 || ( notaryid == 0 && height > Params().Season5StartHeight() ) ) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check proof of work
    if ( (ASSETCHAINS_SYMBOL[0] != 0 || nHeight < Params().KomodoNotaryLowerLimitHeight() 
            || nHeight > Params().KomodoNotaryUpperLimitHeight() ) 
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "getheaders")
    {
        ...
        if (chainActive.LastTip() != 0 && chainActive.LastTip()->nHeight > 100000 && IsInitialBlockDownload())
        {
