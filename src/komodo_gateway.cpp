                ...
                if ( values != 0 && srcvalues != 0 && kmdheights != 0 && otherheights != 0 && baseids != 0 && rmd160s != 0 )
                {
                    ...
                    if ( 0 )
                    {
                        ...
                        printf(">>>>>>> %s: (%s) fiat %.8f kmdheight.%d other.%d -> %s %.8f\n",type=='A'?"approvedA":"issuedX",
                                baseids[n]>=0?komodo_currency(baseids[n]):"???",dstr(p.fiatoshis),kmdheights[n],otherheights[n],coinaddr,dstr(values[n]));
                    }
                }
            }
            else
...
{
    pax_transaction *pax,*pax2,*tmp; 
    int32_t i,ht; 
    int64_t checktoshis; 
    uint64_t seed,total = 0; 
    ...
    else
    {
        HASH_ITER(hh,PAX,pax,tmp)
        {
            ...
            if ( pax->type == 'A' || pax->type == 'D' || pax->type == 'X' )
                str = pax->symbol;
            komodo_state *basesp = komodo_stateptr(str);
...
int32_t komodo_gateway_deposits(CMutableTransaction *txNew,char *base,int32_t tokomodo)
{
    ...
    uint8_t *script,opcode,opret[16384*4],data[16384*4]; 
    int32_t i,baseid,ht,len=0,opretlen=0,numvouts=1; 
    uint64_t available,deposited,issued,withdrawn,approved,redeemed,mask,sum = 0;
        ...
        *script++ = 0xac;
        ...
        else
        {
            ...
            printf(" len.%d vout.%u DEPOSIT %.8f <- pax.%s pending ht %d %d %.8f | ",
                    len,pax->vout,(double)txNew->vout[numvouts].nValue/COIN,base,pax->height,pax->otherheight,dstr(PENDING_KOMODO_TX));
        }
    ...
    if ( numvouts > 1 )
    {
        ...
        len += sym.size()+1; // add 1 for null terminator
        for (i=0; i<len; i++)
            printf("%02x",data[i]);
        printf(" <- data[%d]\n",len);
        ...
        if ( opretlen == 38 ) // any KMD tx
        {
            ...
            if ( strcmp(base,ASSETCHAINS_SYMBOL) == 0 && (kmdheight > 195000 || kmdheight <= height) )
            {
                ...
                if ( komodo_paxcmp(base,kmdheight,value,checktoshis,kmdheight < 225000 ? seed : 0) == 0 )
                {
                    ...
                    {
                        if ( (basesp= komodo_stateptr(base)) != 0 )
                        {
    ...
    else if ( height < 236000 && tokomodo != 0 && opretbuf[0] == 'A' && ASSETCHAINS_SYMBOL[0] == 0 )
    {
        ...
        if ( (n= komodo_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,1)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                ...
                didstats = 0;
                seed = 0;
                checktoshis = komodo_paxprice(&seed,kmdheights[i],komodo_currency(baseids[i]),(char *)"KMD",(uint64_t)values[i]);
                if ( srcvalues[i] == checktoshis )
                {
                    if ( (pax= komodo_paxfind(txids[i],vouts[i],'A')) == 0 )
                    {
                        ...
                        komodo_gateway_deposit(coinaddr,srcvalues[i],komodo_currency(baseids[i]),values[i],&rmd160s[i*20],
                                txids[i],vouts[i],'A',kmdheights[i],otherheights[i],komodo_currency(baseids[i]),kmdheights[i]);
                        if ( (pax= komodo_paxfind(txids[i],vouts[i],'A')) == 0 )
                        ...
                        if ( values[i] != 0 && (basesp= komodo_stateptr(komodo_currency(baseids[i]))) != 0 )
                        {
                    ...
                    {
                        if ( (basesp= komodo_stateptr(komodo_currency(baseids[i]))) != 0 )
                        {
                            basesp->approved += values[i];
                            didstats = 1;
                        }
        ...
        if ( (n= komodo_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,1)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                ...
                if ( (pax= komodo_paxfind(txids[i],vouts[i],'X')) != 0 )
                {
                    ...
                    if ( baseids[i] >= 0 && value != 0 && (basesp= komodo_stateptr(komodo_currency(baseids[i]))) != 0 )
                    {
...
void komodo_passport_iteration()
{
    ...
    FILE *fp; uint8_t *filedata; long fpos,datalen,lastfpos; int32_t baseid,limit,n,ht,isrealtime,expired = 0,refid,blocks,longest;
    komodo_state *sp,*refsp; 
    char *retstr,fname[512]; 
    uint32_t buf[3],starttime; uint64_t RTmask = 0;
