...
#include "komodo_globals.h" // for komodo_baseid()
...
struct komodo_state *komodo_stateptrget(char *base)
{
    ...
    if ( base == 0 || base[0] == 0 || strcmp(base,(char *)"KMD") == 0 )
        return(&KOMODO_STATES[33]); // shortcut, KMD is the default
    else if ( (baseid= komodo_baseid(base)) >= 0 ) // look in the CURRENCIES array
        return(&KOMODO_STATES[baseid+1]); // The +1 here seems strange
    else return(&KOMODO_STATES[0]); // [0] is the ASSETCHAINS_SYMBOL
}
...
void komodo_prefetch(FILE *fp)
{
    ...
    long fpos = ftell(fp); // get the current position
    fseek(fp,0,SEEK_END);
    long fsize = ftell(fp); // get the file size
    if ( fsize > incr ) // if the size of the file is greater than what we want to grab...
    {
