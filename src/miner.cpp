...
CBlockTemplate* CreateNewBlock(const CPubKey _pk, const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake)
{
    ...
    CBlockIndex* pindexPrev = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            CAmount nTxFees = view.GetValueIn(chainActive.LastTip()->GetHeight(),interest,tx)-tx.GetValueOut();
    ...
    try {
        ...
        while (true)
        {
            ...
            if ( ASSETCHAINS_STAKED != 0 )
            {
                ...
                HASHTarget.SetCompact(Params().GetConsensus().mindiff_nbits,&fNegative,&fOverflow);
...
#endif
{
    const char* log_category = "mining";
    LogPrint(log_category, "KomodoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 )
        LogPrint(log_category,"notaryid.%d Mining.%s with %s\n",
                notaryid,ASSETCHAINS_SYMBOL,solver.c_str());
    std::mutex m_cs;
    bool cancelSolver = false;

    // cancel the solver if we hear someone else already solved it
    ...
    try 
    {
        ...
        while (true)
        {
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrint(log_category, "Error in KomodoMiner: Keypool ran out, please call keypoolrefill "
                            "before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrint(log_category, "Error in KomodoMiner: Invalid -mineraddress\n");
                }
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            ...
            if ( ASSETCHAINS_SYMBOL[0] == 0 && notaryid >= 0 )
            {
                        ...
                        || Mining_height > 1000000 )
                {
                    ...
                    if ( nonzpkeys > 0 )
                    {
                        ...
                        LogPrint(log_category,"ht.%i all NN are elegible\n",Mining_height);
                            ...
                            && Mining_height > KOMODO_LASTMINED+64) )
                    {
                        ...
                        LogPrint(log_category, "I am the chosen one for %s ht.%d\n",
            ...
            if ( ASSETCHAINS_STAKED > 0 )
            {
                ...
                if ( ASSETCHAINS_STAKED < 100 )
                    LogPrint(log_category, "Block %d : PoS %d%% vs target %d%% \n",Mining_height,percPoS,
            ...
            while (true)
            {
                ...
                // (x_1, x_2, ...) = A(I, V, n, k)
                LogPrint(log_category, "Running Equihash solver \"%s\" with nNonce = %s\n",
                ...
                std::function<bool(std::vector<unsigned char>)> validBlock =
                        ...
                        &chainparams, &hashTarget_POW, &log_category] (std::vector<unsigned char> soln) 
                {
                    int32_t z; arith_uint256 h; CBlock B;
                    // Write the solution to the hash and compute the result.
                    LogPrint(log_category, "- Checking solution against target\n");
                    ...
                    else
                    {
                        ...
                        {
                            // Need to rebuild block if the found solution for PoS, meets POW
                            // target, otherwise it will be rejected. 
                            if ( ASSETCHAINS_STAKED < 100 
                                    && komodo_newStakerActive(Mining_height,pblock->nTime) != 0 
                                    && h < hashTarget_POW )
                            {
                                LogPrint(log_category, "[%s:%d] PoS block.%u meets POW_Target.%u building new block\n",
                        ...
                        LogPrint(log_category,"[%s:%d] mined block %s\n ",ASSETCHAINS_SYMBOL,Mining_height,
                                B.GetHash().ToString().c_str());
                    }
                    CValidationState state;
                    ...
                    LogPrint(log_category, "proof-of-work found  \n  hash: %s  \ntarget: %s\n", B.GetHash().GetHex(),
                            HASHTarget.GetHex());
                    ...
                    return true;
                }; // end of validateBlock lambda

                // lambda called if the solver cancels the solve
                ...
                if (solver == "tromp" ) 
                {
                    ...
                    for (size_t s = 0; s < eq.nsols; s++) {
                        LogPrint(log_category, "Checking solution %d\n", s+1);
                ...
                else 
                {
                    ...
                    try {
                        ...
                        LogPrint(log_category, "Equihash solver cancelled\n");
                        std::lock_guard<std::mutex> lock{m_cs};
                        cancelSolver = false;
                    }
                }
                ...
                if (vNodes.empty() && chainparams.MiningRequiresPeers())
                {
                    ...
                    {
                        LogPrint(log_category,"no nodes, break\n");
                        break;
                    }
                }
                if ((UintToArith256(pblock->nNonce) & 0xffff) == 0xffff)
                {
                    LogPrint(log_category,"0xffff, break\n");
                    break;
                }
                if (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 60)
                {
                ...
                // we were not successful. Adjust things and loop again
            ...
            } // end of while mining 1 block loop
        } // end of while mining loop
    }
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrint(log_category, "KomodoMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrint(log_category, "KomodoMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
