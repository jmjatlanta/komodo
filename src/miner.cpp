 ...
 */
CBlockTemplate* CreateNewBlock(const CPubKey _pk, const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake, bool includeMempool)
{
    ...
    CBlockIndex* pindexPrev = 0;
    {
        ...
        if (includeMempool)
        {
                ...
                mi != mempool.mapTx.end(); ++mi)
            {
                ...
                uint64_t txvalue = tx.GetValueOut();
                if ( KOMODO_VALUETOOBIG(txvalue) != 0 )
                    continue;
                if ( ASSETCHAINS_SYMBOL[0] == 0 
                        && komodo_validate_interest(tx,nHeight,(uint32_t)pblock->nTime) < 0 )
                {
 ...
 * @param gpucount
 * @param isStake
 * @param includeMempool true to include mempool txs in block
 * @returns the block template
 */
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, int32_t nHeight, int32_t gpucount, bool isStake, bool includeMempool)
{
    ...
    return CreateNewBlock(pubkey, scriptPubKey, gpucount, isStake, includeMempool);
}
