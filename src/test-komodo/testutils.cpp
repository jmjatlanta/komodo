...
std::shared_ptr<CBlock> generateBlock(CWallet* wallet, CValidationState* state = nullptr, bool includeMempool = true); // in mining.cpp
void CalcPoW(CBlock* blk);
 ...
 * @param wallet who gets credit for mining the block
 */
        ...
        bool includeMempool)
{
    ...
    if (wallet == nullptr)
    {
        ...
        block = ::generateBlock(wallet.get(), state, includeMempool);
    return block;
}

std::shared_ptr<CBlock> TestChain::generateBlock(const uint256& prevHash, int prevHeight, 
        std::shared_ptr<CWallet> wallet, CValidationState* validationState, bool includeMempool)
{
    IncrementChainTime();
    CReserveKey reservekey(wallet.get());

    std::unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey,prevHeight,KOMODO_MAXGPUCOUNT, false, includeMempool));
    ...
    pblock->hashPrevBlock = prevHash;
    {
        ...
        txCoinbase.vin[0].scriptSig = (CScript() << prevHeight + 1 << CScriptNum(nExtraNonce)) + COINBASE_FLAGS;
    ...
    CValidationState state;
    if (!ProcessNewBlock(1,prevHeight+1,state, NULL, pblock, true, NULL))
    {
...
bool TestWallet::Sign(CMutableTransaction& tx)
{
    // sign each vin
    for(auto& txIn : tx.vin)
    {
        // find the previous unspent transaction
        CTransaction prevOut;
        uint256 hashBlock;
        if (!myGetTransaction(txIn.prevout.hash, prevOut, hashBlock))
            return false;

        const CScript& scriptPubKey = prevOut.vout[txIn.prevout.n].scriptPubKey;
        SignatureData sigdata;
        // TODO: Properly set consensusbranchid
        CTransaction immutable(tx);
        bool signSuccess = ProduceSignature(TransactionSignatureCreator(this, &immutable, 0, prevOut.vout[txIn.prevout.n].nValue, SIGHASH_ALL), scriptPubKey, sigdata, 0);
...
TransactionInProcess TestWallet::CreateSpendTransaction(std::shared_ptr<TestWallet> to, 
        CAmount amount, CAmount fee, bool commit, bool sign)
{
    ...
    if (!CWallet::CreateTransaction(vecSend, retVal.transaction, retVal.reserveKey, nFeeRequired,
            nChangePosRet, strError, nullptr, sign))
    {
