...
#include "komodo_structs.h" // for pax_transaction
...
pax_transaction *komodo_paxfind(uint256 txid,uint16_t vout,uint8_t type);

pax_transaction *komodo_paxfinds(uint256 txid,uint16_t vout);

pax_transaction *komodo_paxmark(int32_t height,uint256 txid,uint16_t vout,uint8_t type,int32_t mark);

void komodo_paxdelete(pax_transaction *pax);

void komodo_gateway_deposit(char *coinaddr,uint64_t value,const char *symbol,uint64_t fiatoshis,
        uint8_t *rmd160,uint256 txid,uint16_t vout,uint8_t type,int32_t height,int32_t otherheight,
        const char *source,int32_t approved);

int32_t komodo_rwapproval(int32_t rwflag,uint8_t *opretbuf,pax_transaction *pax);
...
{
#define pax_a (*(pax_transaction **)a)
#define pax_b (*(pax_transaction **)b)
    uint64_t aval,bval;
