        ...
        uint64_t *approved,uint64_t *redeemed,const char *base)
{
    ...
    *available = *deposited = *issued = *withdrawn = *approved = *redeemed = 0;
    ...
    {
        if ( (sp= komodo_stateptr(base)) != nullptr )
        {
...
void komodo_gateway_deposit(char *coinaddr,uint64_t value,const char *symbol,uint64_t fiatoshis,uint8_t *rmd160,uint256 txid,
        uint16_t vout,uint8_t type,int32_t height,int32_t otherheight,const char *source,int32_t approved) // assetchain context
{
    struct pax_transaction *pax; uint8_t buf[35]; int32_t addflag = 0; char *s;
    //if ( KOMODO_PAX == 0 )
    //    return;
            ...
            if ( iskomodo != 0 )
            {
                ...
                if ( values != 0 && srcvalues != 0 && kmdheights != 0 && otherheights != 0 && baseids != 0 && rmd160s != 0 )
                {
                    ...
                    if ( 0 )
                    {
                        ...
                        printf(">>>>>>> %s: (%s) fiat %.8f kmdheight.%d other.%d -> %s %.8f\n",type=='A'?"approvedA":"issuedX",
                                baseids[n]>=0?komodo_currency(baseids[n]):"???",dstr(p.fiatoshis),kmdheights[n],otherheights[n],
                                coinaddr,dstr(values[n]));
                    }
                }
            }
            else
...
{
    struct pax_transaction *pax,*pax2,*tmp,*tmp2; char *str; int32_t i,ht; int64_t checktoshis; uint64_t seed,total = 0; struct komodo_state *basesp;
    ...
    HASH_ITER(hh,PAX,pax,tmp)
    {
        pax->ready = 0;
        if ( 0 && pax->type == 'A' )
            printf("%p pax.%s <- %s marked.%d %.8f -> %.8f validated.%d approved.%d\n",pax,pax->symbol,pax->source,pax->marked,
                    dstr(pax->komodoshis),dstr(pax->fiatoshis),pax->validated != 0,pax->approved != 0);
        if ( pax->marked != 0 )
            continue;
        if ( strcmp(komodo_chainsymbol(),pax->symbol) == 0 || pax->type == 'A' )
        {
            if ( pax->marked == 0 )
            {
...
{
    struct pax_transaction *pax,*tmp; uint8_t *script,opcode,opret[16384*4],data[16384*4]; int32_t i,baseid,ht,len=0,opretlen=0,numvouts=1; uint64_t available,deposited,issued,withdrawn,approved,redeemed,mask,sum = 0;
    if ( KOMODO_PASSPORT_INITDONE == 0 )//KOMODO_PAX == 0 ||
        return(0);
    komodo_state *kmdsp = komodo_stateptr("KMD");
    komodo_state *sp = komodo_stateptr();
    ...
    HASH_ITER(hh,PAX,pax,tmp)
    {
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 && (pax_fiatstatus(&available,&deposited,&issued,&withdrawn,&approved,&redeemed,sp->symbol.c_str()) != 0 || available < pax->fiatoshis) )
        {
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 && (strcmp(pax->symbol,sp->symbol.c_str()) != 0 || pax->validated == 0 || pax->ready == 0) )
        {
            if ( strcmp(pax->symbol,ASSETCHAINS_SYMBOL) == 0 )
                printf("pax->symbol.%s != %s or null pax->validated %.8f ready.%d ht.(%d %d)\n",
                        pax->symbol,sp->symbol.c_str(),dstr(pax->validated),pax->ready,kmdsp->CURRENT_HEIGHT,pax->height);
            pax->marked = pax->height;
            continue;
        }
        if ( pax->ready == 0 )
        ...
        *script++ = 0xac;
        ...
        else
        {
            ...
            printf(" len.%d vout.%u DEPOSIT %.8f <- pax.%s pending ht %d %d %.8f | ",
                    len,pax->vout,(double)txNew->vout[numvouts].nValue/COIN,
                    sp->symbol.c_str(),pax->height,pax->otherheight,dstr(PENDING_KOMODO_TX));
        }
    ...
    if ( numvouts > 1 )
    {
        std::string sym = sp->symbol;
        if ( tokomodo != 0 )
            sym = "KMD";
        strcpy( (char *)&data[len], sym.c_str());
        len += sym.size();
        data[len++] = 0;
        for (i=0; i<len; i++)
            printf("%02x",data[i]);
        ...
        if ( opretlen == 38 ) // any KMD tx
        {
            ...
            if ( strcmp(base,ASSETCHAINS_SYMBOL) == 0 && (kmdheight > 195000 || kmdheight <= height) )
            {
                ...
                if ( komodo_paxcmp(base,kmdheight,value,checktoshis,kmdheight < 225000 ? seed : 0) == 0 )
                {
                    ...
                    {
                        if ( (basesp= komodo_stateptr(base)) != nullptr )
                        {
        ...
        if ( strncmp((char *)"KMD",(char *)&opretbuf[opretlen-4],3) != 0 && strncmp(ASSETCHAINS_SYMBOL,(char *)&opretbuf[opretlen-4],3) == 0 )
        {
            if ( (n= komodo_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,0)) > 0 )
            {
                for (i=0; i<n; i++)
                {
                    ...
                    komodo_gateway_deposit(coinaddr,0,ASSETCHAINS_SYMBOL,0,0,txids[i],vouts[i],'I',height,0,
                            komodo_currency(baseids[i]),0);
                    ...
                    if ( (pax= komodo_paxfind(txids[i],vouts[i],'I')) != 0 )
                    {
                        ...
                        if ( (pax2= komodo_paxfind(txids[i],vouts[i],'D')) != 0 && pax2->fiatoshis != 0 && pax2->komodoshis != 0 )
                        {
                            ...
                            if ( pax->didstats == 0 )
                            {
                                if ( (basesp= komodo_stateptr(komodo_currency(baseids[i]))) != nullptr )
                                {
                                    basesp->issued += pax->fiatoshis;
                                    pax->didstats = 1;
                                    pax->height = pax2->height;
                                    pax->otherheight = height;
                                    if ( strcmp( komodo_currency(baseids[i]),"USD") == 0 )
                                        printf("########### %p issuedb %s += %.8f kmdheight.%d %.8f other.%d [%d]\n",
                                                basesp,komodo_currency(baseids[i]),dstr(pax->fiatoshis),pax->height,
                                                dstr(pax->komodoshis),pax->otherheight,height);
                                }
                            }
                        }
                    }
    ...
    else if ( height < 236000 && tokomodo != 0 && opretbuf[0] == 'A' && ASSETCHAINS_SYMBOL[0] == 0 )
    {
        ...
        if ( (n= komodo_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,1)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                ...
                didstats = 0;
                seed = 0;
                checktoshis = komodo_paxprice(&seed,kmdheights[i], komodo_currency(baseids[i]), "KMD", (uint64_t)values[i]);
                if ( srcvalues[i] == checktoshis )
                {
                    if ( (pax= komodo_paxfind(txids[i],vouts[i],'A')) == 0 )
                    {
                        bitcoin_address(coinaddr,60,&rmd160s[i*20],20);
                        komodo_gateway_deposit(coinaddr,srcvalues[i], komodo_currency(baseids[i]),values[i],&rmd160s[i*20],
                                txids[i],vouts[i],'A',kmdheights[i],otherheights[i],
                                komodo_currency(baseids[i]),kmdheights[i]);
                        if ( (pax= komodo_paxfind(txids[i],vouts[i],'A')) == 0 )
                        ...
                        if ( values[i] != 0 && (basesp= komodo_stateptr( komodo_currency(baseids[i]))) != nullptr )
                        {
        ...
        if ( (n= komodo_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,1)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                ...
                if ( (pax= komodo_paxfind(txids[i],vouts[i],'X')) != 0 )
                {
                    ...
                    if ( baseids[i] >= 0 && value != 0 && (basesp= komodo_stateptr( komodo_currency(baseids[i]))) != nullptr )
                    {
                        basesp->redeemed += value;
                        pax->didstats = 1;
                        if ( strcmp( komodo_currency(baseids[i]),ASSETCHAINS_SYMBOL) == 0 )
                            printf("ht.%d %.8f ########### %p redeemed %s += %.8f %.8f kht.%d ht.%d\n",height,
                                    dstr(value),basesp,komodo_currency(baseids[i]),dstr(value),dstr(srcvalues[i]),
                                    kmdheights[i],otherheights[i]);
                    }
                }
...
void komodo_passport_iteration()
{
    ...
    FILE *fp; uint8_t *filedata; long fpos,datalen,lastfpos; int32_t baseid,limit,n,ht,isrealtime,expired,refid,blocks,longest; 
    struct komodo_state *sp,*refsp; 
    char *retstr,fname[512];
    uint32_t buf[3],starttime; uint64_t RTmask = 0; //CBlockIndex *pindex;
    expired = 0;
    ...
    for (baseid=32; baseid>=0; baseid--)
    {
        ...
        sp = 0;
        isrealtime = 0;
        const char *base = komodo_currency(baseid);
                ...
                else if ( (fp= fopen(fname,"rb")) != 0 && sp != 0 )
                {
                    ...
                    if ( ftell(fp) > lastpos[baseid] )
                    {
                        ...
                        if ( 0 && lastpos[baseid] == 0 && strcmp(symbol,"KMD") == 0 )
                            printf("from.(%s) lastpos[%s] %ld isrt.%d\n",ASSETCHAINS_SYMBOL, komodo_currency(baseid),
                                    lastpos[baseid],komodo_isrealtime(&ht));
                    }
                    fclose(fp);
