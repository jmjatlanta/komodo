        ...
        if ( n > 0 )
        {
            ...
            if ( (err= rogue_isvalidgame(cp,gameheight,tx,buyin,maxplayers,gametxid,1)) == 0 )
            {
                ...
                if ( playertxid != zeroid )
                {
                    ...
                    {
                        std::vector<std::pair<uint8_t, vscript_t>>  oprets;
                        if ( (funcid= CCTokens::DecodeTransactionOpRet(playertx.vout.back().scriptPubKey, e, tid, voutPubkeys, oprets)) != 0)
                        {  // if token in the opret
                            ...
                            rawtx = FinalizeCCTx(0, cp, mtx, mypk, txfee,
                                CCTokens::EncodeTransactionOpRet(tid, voutPubkeysEmpty /*=never spent*/, std::make_pair(OPRETID_ROGUEGAMEDATA, vopretRegister)));
                        }
                    }
                }
                if ( didtx == 0 )
